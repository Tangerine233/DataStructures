...
void view_poly(const polynomial& test)
{
    ...
    bool firstTerm = true;
    if (test.degree()!=0){
        for (size_t i=test.degree(); i>=0; i--){
            if (firstTerm){
                if (test.coefficient(i)!=0) {
                    if (i==0) cout<<test.coefficient(i);
                    else cout<<test.coefficient(i)<<"x^"<<i;
                    firstTerm = false;
                }
            }
            else{
    ...
    else if (test.degree()==0) cout<<test.coefficient(0);
    else cout<<"0";
